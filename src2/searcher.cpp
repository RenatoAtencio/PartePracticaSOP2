...
int main() {
    ...
    // Crear el socket del cliente (Programa 1)
    clientSocket = socket(AF_INET, SOCK_STREAM, 0);
    if (clientSocket == -1) {
        perror("Error al crear el socket del cliente (Programa 1)");
        exit(EXIT_FAILURE);
    }
    ...
    string credentials = "searcher,searcher"; // Credenciales de Programa 1
    send(clientSocket, credentials.c_str(), credentials.length(), 0);
    cout << "Programa 1 - Credenciales enviadas al servidor" << endl;
